...
| **Real Google map** — Driver home + **driver active-trip** **+ Rider** home (`EvcGoogleMap`, Dubai-centered) + `EvcLocation` GPS w/ UAE service-region fallback | **RID-03 · MAT** | ✅ **Slices 1–4 of 4** — real map tiles + dev-from-India fallback ✅; **real route polyline + road distance/drive-time (Directions API)** ✅; **Places autocomplete (Slice 2)** ✅; **live moving driver dot (Slice 4)** ✅ — driver streams position (`publishLocation`, real GPS or dev simulation) → rider subscribes (`driverLocationStream`, RLS-scoped Realtime) → car marker moves live. **Admin live map** ✅ — real `EvcGoogleMap` + `adminFleetLiveProvider` (Realtime `driver_locations` stream) → driver dots move live (azure = on a trip, green = available) + ongoing-trips sheet |
...
| **Charging-session lifecycle — Phase 1+2** | **CHG-01·02·03·07 · ADM-05** | ✅ session spine: start → simulated live meter (kWh + cost + battery%/range) → **grace → idle penalty** → stop & settle → receipt; battery drains per trip + refills on charge · **idle penalty + grace (CHG-03)** + **Watanya 30% margin** on settle (CHG-07) · **reserve/queue (CHG-02)** — `charging_queue` + `join`/`leave`/`_promote_queue`; driver reserve-a-stall / join-queue with live position, a freed stall promotes the next, realtime banner · **admin station mgmt (ADM-05)** — live charger status / available / queue + editable per-station rate (`admin_set_station_rate`). **Simulated:** kWh/plug/battery (no OCPP). *(Cross-sell CHG-04 = customer-app only; N/A for driver charging.)* |
