...
| **Charging-session lifecycle — Phase 1** | **CHG-01 · CHG-07** | ✅ real session spine: `start_charging`/`update_charging`/`stop_charging` RPCs + `charging_sessions` (status/kWh/cost/vat/rate) → driver UI **start → simulated live meter (kWh + running cost + battery %) → stop & settle** (cost = kWh × per-station rate + VAT) → receipt; stall taken/freed; driver offline while charging. **Simulated:** the kWh meter / plug / battery (no OCPP hardware). **Phase 2:** reserve/queue · idle penalty · Watanya 30% margin · Meeza discount · cross-sell ride · admin station mgmt (ADM-05) |
