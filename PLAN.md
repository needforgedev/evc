...
- ✅ **Real trips (Milestone A)** — Rider `request_ride` → server prices it + **range/tier/compliance/radius-aware dispatch** → Driver realtime **offer → accept → arrived → start → complete**; writes `payments`, bumps **driver earnings**, updates **Admin live-map/Trips/KPIs**. CO₂ per trip + fares from the pricing table.
  - ✅ **No-availability handling** — `dispatch_trip` radius cap (`pricing.dispatch_radius_km`); `nearby_tiers()` powers per-tier "N min away" / greyed-out empty tiers on booking; no-match → orphan canceled + **alternatives sheet** + 45s live-screen timeout (no infinite "Finding your EV…").
